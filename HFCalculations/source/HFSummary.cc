...
void summarize(const char *filename_){
    ...
    states.push_back(Ex);
        ...
        for (int i=0;i<nRows;i++){
            ...
            dummy.push_back(jj); dummy.push_back(bhgs); dummy.push_back(bh854); 
                dummy.push_back(bh1561); dummy.push_back(bh2004);
    ...
    for(int i=0;i<nSpins;i++){
        ...
        hs[i]->Add(hCalc[0][i]); hs[i]->Add(hCalc[2][i]); hs[i]->Add(hCalc[1][i]);
