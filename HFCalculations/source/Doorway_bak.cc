...
void doorway(const char *filename_){
    ...
    for (int f=0;f<nFiles;f++){
        ...
        double t_itg;
        for(int k=0;k<3;k++) {
            hrel[f][k] = new TH1F(Form("h%d_j%d",k,spin),Form("h%d_j%d",k,spin),nBins,lX,hX);
            hrel[f][k]->SetFillColor(lc[k]); 
            hrel[f][k]->SetLineColor(lc[k]);

            habs[f][k] = new TH1F(Form("habs%d_j%d",k,spin),Form("habs%d_j%d",k,spin),nBins,lX,hX);
        ...
        for(int i=1;i<=nBins;i++){
           ...
           for (int k=0;k<3;k++){ hrel[f][k]->SetBinContent(i,itg[k]/t_itg); habs[f][k]->SetBinContent(i,itg[k]); itg[k]=0.0;}
        }
    }
    ...
    if(cType==1){
        ...
        for(int i=0;i<nFiles;i++){
            ...
            hs[i]->Add(hrel[i][0]); hs[i]->Add(hrel[i][2]); hs[i]->Add(hrel[i][1]);
